...
namespace UserStraightSegmentCalculationServers.Pipe
{
  ...
  public class PlumbingFixtureFlowServer : IPipePlumbingFixtureFlowServer
  {
    ...
    private void initilizeflushValves()
    {
      ...
      //the key is Fixture Units
      //the value is Flow (gallon (U.S.) per minute (gpm) (gal/min))
      flushValves.Add( new KeyValuePair<double, double>( 1, 3.0 ) );
      flushValves.Add( new KeyValuePair<double, double>( 2, 5.0 ) );
      flushValves.Add( new KeyValuePair<double, double>( 3, 6.5 ) );
      flushValves.Add( new KeyValuePair<double, double>( 4, 8.0 ) );
      //includes flush tank conversions below 5 FU for flush valve systems
...
{
    return new Guid("A3C5483F-0F42-4C2A-A37B-9FCB1B40F6C4");
}
    ...
    public System.String GetName()
        {
      return "PE/CPD Approved (Flush Valve ≤ 5 FU)";
    }
    ...
    {
      return "This FU-Flow rate conversion method still applies 2021 IPC-based Hunter's Curve interpolated values, however the predominantly flush valve table has been extended to consider FU values below 5FU - leading to properly sized piping. Revit, per the IPC, converts all FU values below 5 FU to 15 GPM, which is not correct. This method extends the conversion from FU to flow rate below 5FU, leading to more accurate flow rates.";
    }
  ...
  //  }
    ...
    {
      return "This calculation method overrides Revit’s default plumbing fixture flow logic by applying corrected flow rates for low-demand conditions in predominantly flush valve systems. It uses values from the 2021 International Plumbing Code (IPC) Table 103.3(3) for converting Fixture Units (FUs) to flow (GPM). However, the IPC table omits values for systems under 5 FUs with flush valves, and Revit defaults these to an inflated 15 GPM, leading to oversized pipes.This add-in resolves that by applying the flush tank conversion table for all cases under 5 FUs, even when the system is classified as predominantly flush valve. This adjustment results in more accurate flow calculations, corrected pipe sizing, realistic velocity, and pressure drop—improving design accuracy and aligning with CPD and PE best practices.";
    }
  }
}
